  ...
  Future<void> _submitComment() async {
    ...
    if (!request.loggedIn) {
      ...
      if (result != true || !updatedRequest.loggedIn) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Silakan login terlebih dahulu untuk menambahkan komentar')),
          );
        }
        return;
      }
    ...
    try {
      ...
      print('[DEBUG] Submitting comment - loggedIn: ${currentRequest.loggedIn}');
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Gagal menambah komentar: ${e.toString().replaceAll('Exception: ', '')}'),
  ...
  Future<void> _submitRating(int rating) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Gagal menambah rating: ${e.toString().replaceAll('Exception: ', '')}'),
            duration: const Duration(seconds: 3),
          ),
        );
      }
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    boxShadow: [
                                      BoxShadow(
                                        color: _getDifficultyGradient(video.difficulty).colors.first.withOpacity(0.3),
                                        blurRadius: 4,
                            ...
                            Row(
                              ...
                              children: List.generate(5, (index) {
                                ...
                                return _AnimatedStarButton(
                                  rating: rating,
                                  isSelected: _selectedRating != null && rating <= _selectedRating!,
                                  onTap: () {
                                    setState(() {
                                ...
                                if (comments.isEmpty) {
                                  return Center(
                                    child: Padding(
                                      ...
                                      child: Column(
                                        children: [
                                          Icon(Icons.comment_outlined, size: 48, color: textSecondary),
                                          const SizedBox(height: 12),
                                          Text(
